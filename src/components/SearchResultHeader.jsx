    ...
    return <div className="p-[15px] pb-0 md:pr-5 md:pl-20 md:pt-7 border-b border-[#ebebeb] flex md:block flex-col items-center sticky top-0 bg-white">
        <div className="flex items-center justify-between w-full">
            <div className="flex items-center grow">
                <Link to='/'>
                    <img className="hidden md:block w-[100px] mr-10" src={Logo} alt='logo' />
                </Link>
                <SearchInput from="searchResult" />
            </div>
        </div>
        <div className="flex mf-[12px] mt-3">
            {menu.map((menu, index) => (
                <span key={index} className={`flex items-center p-3 text-[#5f6368] cursor-pointer relative ${selectedMenu === menu.name ? "text-green-700" : ""}`}
                    ...
                    {selectedMenu === menu.name && (
                        <span className="h-[3px] w-[calc(100%-20px)] absolute bg-green-700 bottom-0 left-[10px]"></span>
                    )}
                </span>
            ))}
        </div>
